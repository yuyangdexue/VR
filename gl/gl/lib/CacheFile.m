...
+ (NSString *)cache_file_get:(NSString *)fileName {
  ...
  NSFileManager *fm = [NSFileManager defaultManager];
  if ([fm fileExistsAtPath:path]) {
    return [[NSString alloc] initWithData:[fm contentsAtPath:path]
...
// 缓存文件 读取
+ (NSString *)cache_file_get:(NSString *)fileName
               expiredSecond:(NSInteger)expiredSecond {
  if (!fileName || fileName.length == 0) {
    return @"";
  }
  NSString *path = [self cache_file_path:fileName];
  NSFileManager *fm = [NSFileManager defaultManager];
  if ([fm fileExistsAtPath:path] &&
      ![[self class] cache_file_expired:fileName expiredSecond:expiredSecond]) {
    return [[NSString alloc] initWithData:[fm contentsAtPath:path]
                                 encoding:NSUTF8StringEncoding];
  }
  [[self class] cache_file_delete:fileName];
  return @"";
}
...
// 缓存文件 是否过期
// expiredSecond 过期多少秒
+ (BOOL)cache_file_expired:(NSString *)fileName
             expiredSecond:(NSInteger)expiredSecond {
  if (!fileName || fileName.length == 0) {
    return YES;
  }
  ...
  NSError *err;
  NSDictionary *fileAttributes = [fm attributesOfItemAtPath:path error:&err];
