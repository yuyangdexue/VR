...
NSString *const SVProgressHUDDidTouchDownInsideNotification =
...
@property(nonatomic, strong) SVIndefiniteAnimatedView *indefiniteAnimatedView2;
...
+ (SVProgressHUD *)sharedView {
  ...
  dispatch_once(&once, ^{
      ...
      sharedView.center=CGPointMake(kDeviceWidth/2, kDeviceHeight/2);
     sharedView.transform =CGAffineTransformMakeRotation(degreesToRadians(90));
  });
  return sharedView;
}


+ (void)isVerticalScreen:(BOOL) isVertical
{
    
    
    if (isVertical) {
        [self sharedView].transform =CGAffineTransformIdentity;
        [self sharedView].center=CGPointMake(kDeviceWidth/2, kDeviceHeight/2);
        [self sharedView].transform =CGAffineTransformMakeRotation(degreesToRadians(0));
    }
    else
    {
        ...
        [self sharedView].transform =CGAffineTransformIdentity;
        [self sharedView].center=CGPointMake(kDeviceWidth/2, kDeviceHeight/2);
       [self sharedView].transform =CGAffineTransformMakeRotation(degreesToRadians(90));
    }
}
#pragma mark - Setters
...
+ (void)showProgress:(float)progress status:(NSString *)status {
  ...
  [self showProgress:progress
              status:status
            maskType:SVProgressHUDDefaultMaskType];
}
...
+ (void)showImage:(UIImage *)image
  ...
  NSTimeInterval displayInterval =
      [[self sharedView] displayDurationForString:string];
  [[self sharedView] showImage:image
                        status:string
...
- (id)initWithFrame:(CGRect)frame {
  if ((self = [super initWithFrame:frame])) {
    ...
    if ([[UIImage class]
      ...
      SVProgressHUDInfoImage =
          [[UIImage imageNamed:@"SVProgressHUD.bundle/info"]
              imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
      SVProgressHUDSuccessImage =
          [[UIImage imageNamed:@"SVProgressHUD.bundle/success"]
              imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
      SVProgressHUDErrorImage =
          [[UIImage imageNamed:@"SVProgressHUD.bundle/error"]
              imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    } else {
      SVProgressHUDInfoImage =
          [UIImage imageNamed:@"SVProgressHUD.bundle/info"];
...
- (void)drawRect:(CGRect)rect {
  ...
  case SVProgressHUDMaskTypeGradient: {
    ...
    CGFloat freeHeight =
        CGRectGetHeight(self.bounds) - self.visibleKeyboardHeight;

    CGPoint center =
        CGPointMake(CGRectGetWidth(self.bounds) / 2, freeHeight / 2);
    float radius =
...
- (void)updatePosition {
  ...
  BOOL progressUsed = (self.progress != SVProgressHUDUndefinedProgress) &&
                      (self.progress >= 0.0f);
  ...
  if (string) {
    ...
    if ([string respondsToSelector:@selector(boundingRectWithSize:
      ...
      if ([string respondsToSelector:@selector(sizeWithAttributes:)])
        stringSize = [string sizeWithAttributes:@{
          NSFontAttributeName :
              [UIFont fontWithName:self.stringLabel.font.fontName
                              size:self.stringLabel.font.pointSize]
        }];
      else
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated"
        stringSize = [string sizeWithFont:self.stringLabel.font
                        constrainedToSize:CGSizeMake(200.0f, 300.0f)];
#pragma clang diagnostic pop
  ...
  if (string)
    self.imageView.center =
        CGPointMake(CGRectGetWidth(self.hudView.bounds) / 2, 36.0f);
  else
    self.imageView.center =
        CGPointMake(CGRectGetWidth(self.hudView.bounds) / 2,
  ...
  if (string) {
    ...
    CGPoint center = CGPointMake((CGRectGetWidth(self.hudView.bounds) / 2),
                                 CGRectGetHeight(self.hudView.bounds) / 2);
    self.indefiniteAnimatedView.center = center;
    [self.indefiniteAnimatedView startAnimating];
...
- (void)positionHUD:(NSNotification *)notification {
  ...
  // no transforms applied to window in iOS 8, but only if compiled with iOS 8
  // sdk as base sdk, otherwise system supports old rotation logic.
  ...
  if (notification) {
    ...
    CGRect keyboardFrame = [
        [keyboardInfo valueForKey:UIKeyboardFrameBeginUserInfoKey] CGRectValue];
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
  ...
  self.hudView.center =
      CGPointMake(newCenter.x + self.offsetFromCenter.horizontal,
...
- (void)overlayViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent *)event {
  ...
  if (CGRectContainsPoint(self.hudView.frame, touchLocation)) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:SVProgressHUDDidTouchDownInsideNotification
                      object:event];
  }
}
...
- (void)showProgress:(float)progress
  ...
  if (!self.overlayView.superview) {
    ...
    // Ensure that overlay will be exactly on top of rootViewController (which
  ...
  if (progress >= 0) {
    ...
    //    UIActivityIndicatorView *iview = [[UIActivityIndicatorView alloc]
    //        initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
    //    iview.frame = CGRectMake(0, 0, 60, 60);
    //    [self.hudView addSubview:iview];
  ...
  if (self.alpha != 1 || self.hudView.alpha != 1) {
    ...
    self.hudView.transform =
        CGAffineTransformScale(self.hudView.transform, 1.3, 1.3);
    ...
    [UIView animateWithDuration:0.15
        ...
        animations:^{
            ...
            if (self.isClear) // handle iOS 7 and 8 UIToolbar which not answers
                              // well to hierarchy opacity change
...
- (void)dismiss {
  ...
  [UIView animateWithDuration:0.15
      ...
      animations:^{
          self.hudView.transform =
              CGAffineTransformScale(self.hudView.transform, 0.8f, 0.8f);
...
- (UIActivityIndicatorView *)indefiniteAnimatedView {
  if (nil == _indefiniteAnimatedView) {
    _indefiniteAnimatedView = [[UIActivityIndicatorView alloc]
        initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
    _indefiniteAnimatedView.frame = CGRectMake(0, 0, 100, 100);
//    _indefiniteAnimatedView.transform = CGAffineTransformMakeScale(1.4, 1.4);
    //    [_indefiniteAnimatedView sizeToFit];
  }
  return _indefiniteAnimatedView;
}

- (SVIndefiniteAnimatedView *)indefiniteAnimatedView2 {
  if (_indefiniteAnimatedView2 == nil) {
    _indefiniteAnimatedView2 =
        [[SVIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
    _indefiniteAnimatedView2.strokeThickness = SVProgressHUDRingThickness;
    _indefiniteAnimatedView2.strokeColor = SVProgressHUDForegroundColor;
    _indefiniteAnimatedView2.radius = self.stringLabel.text
                                          ? SVProgressHUDRingRadius
                                          : SVProgressHUDRingNoTextRadius;
    [_indefiniteAnimatedView2 sizeToFit];
  }
  return _indefiniteAnimatedView2;
}

- (CAShapeLayer *)ringLayer {
  if (!_ringLayer) {
    CGPoint center = CGPointMake(CGRectGetWidth(_hudView.frame) / 2,
                                 CGRectGetHeight(_hudView.frame) / 2);
...
- (BOOL)isClear { // used for iOS 7 and above
  return (self.maskType == SVProgressHUDMaskTypeClear ||
...
- (CGFloat)visibleKeyboardHeight {
  ...
  for (__strong UIView *possibleKeyboard in [keyboardWindow subviews]) {
    ...
    } else if ([possibleKeyboard
                   isKindOfClass:NSClassFromString(
                                     @"UIInputSetContainerView")]) {
      for (__strong UIView *possibleKeyboardSubview in
           [possibleKeyboard subviews]) {
        if ([possibleKeyboardSubview
                isKindOfClass:NSClassFromString(@"UIInputSetHostView")]) {
