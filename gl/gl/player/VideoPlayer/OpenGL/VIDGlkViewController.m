...
#define ROLL_CORRECTION ES_PI/2
...
- (void)viewDidLoad
{
    ...
    if (!self.context) {
                UIAlertView *alter=[[UIAlertView alloc]initWithTitle:nil message:@"该设备不支持OpenGLES" delegate:self cancelButtonTitle:@"知道了！" otherButtonTitles:nil, nil];
                [alter show];
    }
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(noti:) name:@"overture" object:nil];
//       [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(noticationx:) name:@"fingerRotationX" object:nil];
//       [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(noticationy:) name:@"fingerRotationY" object:nil];
    
   // [self startDeviceMotion];
    
}

- (void)noticationx:(NSNotification*)noti
{
    _fingerRotationX=[((NSNumber *)[noti object]) floatValue];
   
}

- (void)noticationy:(NSNotification*)noti
{
    _fingerRotationY=[((NSNumber *)[noti object]) floatValue];
}
- (void)noti:(NSNotification*)noti
{
    _overture=[((NSNumber *)[noti object]) floatValue];
...
- (void)update
{
    float aspect = fabsl(self.view.bounds.size.width / self.view.bounds.size.height);
    ...
    if(_isUsingMotion)
    {
        ...
        if (d != nil) {
            ...
            if(YES)
            {
                ...
                modelViewMatrix = GLKMatrix4RotateZ(modelViewMatrix,cYaw+888);
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect
{
    ...
    CVReturn err;
	if (pixelBuffer != NULL) {
		long frameWidth = CVPixelBufferGetWidth(pixelBuffer);
		long frameHeight = CVPixelBufferGetHeight(pixelBuffer);
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    _mfingerRotationX=_fingerRotationX;
    _mfingerRotationY=_fingerRotationY;
